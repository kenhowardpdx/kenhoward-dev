...
const getPostUrlFromPath = (path: string, postsPath: string, prefix: string): string => {
  const [year, month, day, ...postPath] = path.slice(postsPath.length + 1, path.lastIndexOf('.')).split('-')
